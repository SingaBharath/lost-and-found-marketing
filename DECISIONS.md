...
## 1. Why this presentation strategy over the obvious alternatives?

Instead of relying on standard template layouts or generic stock photography of smiling people, I chose a **live interactive demo architecture**:
...
- **What I'd build with a full week**:
  ...
  3. Add an interactive WebGL or Canvas-based 3D cluster visualizer showing vector space distance embeddings.
...
- **Personal Verification & Modifications**:
  ...
  - **Dark mode contrast & theme consistency**: Hand-audited text contrast and background layer values across both light (`bg-slate-50`, `text-slate-900`) and dark (`bg-slate-950`, `text-slate-100`) modes to guarantee zero half-dark or low-contrast elements.
  ...
  - **Easter Egg Implementation**: Added a Konami code event listener (`↑ ↑ ↓ ↓ ← → ← → b a`) and DevTools console log badge.
